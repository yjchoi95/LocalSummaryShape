function [Xn,muq] = FindWeightedElasticMean(q,lambda)
%lambda: weights for each shape
Niter=30;
n=size(q,3);
% %figure(1); clf; 
% for i=1:n
%     X = ReSampleCurve(Data(:,:,i),NResampled);
%     q(:,:,i) = curve_to_q(X);
% end
...
for i=1:n
    [qbest] = Find_Rotation_and_Seed_unique(mu,q(:,:,i),1); % q with optimal rotation and reparam. to the mu updated above
