function Path_Plot(alpha,p2n,view_az_el,dt)

%colorspecstr = ['b','m','k','r','g','b','k','m','g','r','g','b','k'];
[n,T,k] = size(alpha);
stp = k-1;
colorspecstr = [linspace(1, 0, stp+1)', zeros(stp+1, 1), linspace(0, 1, stp+1)']; 
...
%         plot(ft(1,:), ft(2,:),col); axis equal;   axis tight; hold on; axis equal; axis off; axis tight;
        z = plot(ft(1,:),ft(2,:),'Color',colorspecstr(tau,:),...
            'MarkerFaceColor',colorspecstr(tau,:),'MarkerSize',1,'LineWidth',2); axis equal; hold on;axis equal; axis off; axis tight;  
        %set(z,'LineWidth',3);
    else
        if(n == 3)
            f1(1,:,tau) = ft(1,:);
