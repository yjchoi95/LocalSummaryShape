...
folder_name = strcat('simul_independent');
...
% get independent summary
parfor i = 1:numRefs
    disp(i);
    s0         = ref(i, :);    
    
    tic;
    [X_s0,q_s0,~,~,delta_norm,~,~,q] = ...
        FindElasticMean_extrinsic(data_matrix_local{i});
    toc;
    ...
    psamp_out_pc1{i}   = curvePCA(q, q_s0, ...
        1, strcat(folder_name,'/PC1_',num2str(i),'.png'));
    psamp_out_pc2{i}   = curvePCA(q, q_s0, ...
        2, strcat(folder_name,'/PC2_',num2str(i),'.png'));
    psamp_out_pc3{i}   = curvePCA(q, q_s0, ...
        3, strcat(folder_name,'/PC3_',num2str(i),'.png'));
end

% for i = 1:4
%     [~, K_out{i}] = curvePCA(out.q{i},out.q_s0{i}, ...
%         1, strcat('tmp.png'));
% end
% 
% for i = 1:4
...
psamp_mean = zeros(size(out.q{1},1),size(out.q{1},2),length(out.q)); %zeros(2,size(data_matrix,2),size(ref,1));
midpoint = floor(size(psamp_mean,3)/2) + 1;
