function S = variogram(x,elastic_dist,varargin)
...
% error checking
if size(elastic_dist,1) ~= size(x,1)
...
minx = min(x,[],1);
maxx = max(x,[],1);
% maximum spatial distance to be considered in the variogram
...
% 1st, 2nd col in iid: indices of spatial location in x
% 3rd col in iid: euclidean distance between spatial location iid(,1) and
% iid(,2) such that the distance is smaller than maxd/2.
iid = distmat(x,params.maxdist);

% calculate squared difference between values of coordinate pairs
% lam      = (y(iid(:,1))-y(iid(:,2))).^2;
% get indices in elastic_distance matrix for which the spatial distance is smaller
% than maxd/2 and square the elastic distance
lam       = elastic_dist(sub2ind(size(elastic_dist), iid(:,1), iid(:,2))).^2;
...
if params.anisotropy 
    ...
    % bin theta
    [ntheta, ~, ixtheta] = histcounts(theta, thetaedges);
...
switch params.type
    case {'default','gamma'}
        ...
        % count the number of observations in each bin created by edges
        % nedge: number of observations that are contained in each bin
        % ixedge: index of edge in which the iid(,3) are contained
        [nedge, ~, ixedge] = histcounts(iid(:,3), edges);
...
if params.plotit
    ...
    if ~params.anisotropy
        figure;plot(S.distance,S.val,marker);
